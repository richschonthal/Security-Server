...
CClientConnectThread::CClientConnectThread(CClientSubSystem* pSub)
		: CTypedThreadObject<CSecuritySystem, CClientSubSystem>(pSub)
		, m_pSocket(NULL)
	{
	}
	...
	if(m_pSocket != NULL && GetState() == STATE_RUN)
		...
		if(m_pSocket->Listen(5))
			...
			if(m_pSocket->Accept(socket))
				{
				GetSystem()->GetClientSubSystem()->PostNextThreadMessage(CClientSubSystem::MSG_NEWCONNECTION, (WPARAM) socket.Detach(), NULL);
				::Sleep(10);
				}
			}
		}
