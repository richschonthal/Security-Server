...
//--------------------------------------------------------------------------------
CClientHandlerThread::CClientHandlerThread(CClientSubSystem* pParent)
		: CTypedThreadObject<CSecuritySystem, CClientSubSystem>(pParent)
	{
	}
...
void CClientHandlerThread::OnMessage(UINT nMsg, WPARAM hSocket, LPARAM)
	...
	if(! socket.Send(pMsg, CSmallSocket::WAITFORWOULDBLOCK, 2000))
		{
		GetSystem()->GetIOSubSystem()->FormatOutput(IOMASK_ERR, "1001 socket error %ld", ::WSAGetLastError());
		return;
		}
	...
	switch(nCmd)
		...
		default:
			GetSystem()->GetIOSubSystem()->Output(IOMASK_ERR|IOMASK_CONST, "Illegal command received");
			break;
		}
	}
...
bool CClientHandlerThread::DoBackupServerConnect(const CCertificate& cert, CSmallSocket& socket)
	...
	if(! socket.GetPeerName(sAddr, nPort))
		{
		GetSystem()->GetIOSubSystem()->Output(IOMASK_ERR|IOMASK_CONST, "Error obtaining client IP address");
		return false;
		}
	...
	if(! socket.Send(sTemp, CSmallSocket::WAITFORWOULDBLOCK, 10000))
		{
		GetSystem()->GetIOSubSystem()->Output(IOMASK_ERR|IOMASK_CONST, "Send error DBCS 1");
		return false;
		}
...
bool CClientHandlerThread::DoNewCertificate(const CCertificate& cert, CSmallSocket& socket)
	...
	CCertificateMaster* pCert = GetSystem()->GetDBSubSystem()->IssueCertificate(cert, sAddr);
	if(pCert == NULL)
		{
		GetSystem()->GetIOSubSystem()->Output(IOMASK_1|IOMASK_DEBUG|IOMASK_CONST, "IssueCertificate returns NULL (1)");
		return false;
		}
	...
	if(socket.Send(temp, nLen, CSmallSocket::WAITFORWOULDBLOCK, 10000) != nLen)
		{
		GetSystem()->GetIOSubSystem()->Output(IOMASK_1|IOMASK_DEBUG|IOMASK_CONST, "error certificate not sent");
		return false;
		}

	GetSystem()->GetIOSubSystem()->FormatOutput(IOMASK_1|IOMASK_DEBUG, "issused certificate %ld", pCert->GetId());
	return true;
	}
...
bool CClientHandlerThread::DoAquireToken(CCertificate& cert, CSmallSocket& socket)
	...
	if(! socket.GetPeerName(sAddr, nInPort))
		{
		GetSystem()->GetIOSubSystem()->Output(IOMASK_ERR, "Error obtaining client IP address");
		return false;
		}

	CCertificateMaster* pCert = GetSystem()->GetDBSubSystem()->FindCertMaster(cert);

	// only issue tokens when we are serving
	if(GetSystem()->IsServing() && pCert != NULL)
		GetSystem()->GetDBSubSystem()->IssueToken(pCert, cert.GetCommandParam());
...
bool CClientHandlerThread::DoRefreshCertificate(const CCertificate& cert, CSmallSocket& socket)
	...
	if(GetSystem()->IsServing())
		pCert = GetSystem()->GetDBSubSystem()->RefreshCertificate(cert, sAddr);
...
bool CClientHandlerThread::DoReleaseCertificate(const CCertificate& cert, CSmallSocket& socket)
	...
	if(nTokenId == 0)
		...
		if(! socket.GetPeerName(sAddr, nInPort))
			...
			GetSystem()->GetDBSubSystem()->ReleaseCertificate(cert, sAddr);
		}
	else
		GetSystem()->GetDBSubSystem()->ReleaseToken(cert, nTokenId);
