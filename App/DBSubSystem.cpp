...
CDBSubSystem::CDBSubSystem(CSecuritySystem* pParent)
		: CTypedSubSystem<CSecuritySystem>(pParent)
		, m_nNextCertId(0)
		, m_nEmailInterval(10)
	{
...
CCertificateMaster* CDBSubSystem::IssueCertificate(const CCertificate& rCert, LPCTSTR pIP)
	...
	if(! GetSystem()->IsServing())
		{
		GetSystem()->GetIOSubSystem()->Output(IOMASK_1|IOMASK_DEBUG|IOMASK_CONST, "IssueCertificate (1)");
		return NULL;
		}
	...
	if(rCert.GetCommand() != CCertificateCommands::CMD_NEW)
		{
		GetSystem()->GetIOSubSystem()->Output(IOMASK_1|IOMASK_DEBUG|IOMASK_CONST, "IssueCertificate (2)");
		return NULL;
		}
	...
	if(nTokenId == 0)
		{
		GetSystem()->GetIOSubSystem()->Output(IOMASK_1|IOMASK_DEBUG|IOMASK_CONST, "IssueCertificate (5)");
		return NULL;
		}
	...
	if(! lock.Lock(30000))
		{
		GetSystem()->GetIOSubSystem()->Output(IOMASK_1|IOMASK_DEBUG|IOMASK_CONST, "IssueCertificate (3) must be busy or deadlocked");
		return NULL;
		}
	...
	if(! IssueToken(pCert, nTokenId))
		{
		delete pCert;
		GetSystem()->GetIOSubSystem()->Output(IOMASK_1|IOMASK_DEBUG|IOMASK_CONST, "IssueCertificate (4)");
		return NULL;
		}
...
bool CDBSubSystem::ReleaseCertificate(const CCertificate& rCert, LPCTSTR pIP)
	...
	if(pCert != NULL)
		...
		GetSystem()->GetIOSubSystem()->FormatOutput(IOMASK_1, "Released certificate %d from %s at %s\n",
...
bool CDBSubSystem::IssueToken(CCertificateMaster*& pCert, ULONG nTokenId)
	...
	if(pDB == NULL)
		{
		GetSystem()->GetIOSubSystem()->Output(IOMASK_CONST|IOMASK_ERR, "Program error - Token DB == null");
		return false;
		}
	...
	if(! dbLock.Lock(10000))
		{
		GetSystem()->GetIOSubSystem()->Output(IOMASK_CONST|IOMASK_ERR, "Program error - can't lock the token DB");
		return false;
		}
	...
	if(pTokenInfo == NULL)
		...
		GetSystem()->GetIOSubSystem()->FormatOutput(IOMASK_1|IOMASK_CONST|IOMASK_DEBUG, "IssueCertificate Status 5 (%d)", nTokenId);
		return false;
		}
	...
	if(pTokenInfo->m_nInUse == pTokenInfo->m_nMax)
		...
		GetSystem()->GetIOSubSystem()->Output(IOMASK_CONST|IOMASK_ERR, "No more tokens available");
		return false;
		}
	...
	if(! GetCertMasters()->Find(pCert->GetId()))
		if(! GetCertMasters()->Add(pCert))
			{
			GetSystem()->GetIOSubSystem()->Output(IOMASK_CONST|IOMASK_ERR, "Program error - can't add certificate to internal database");
			return false;
			}
...
bool CDBSubSystem::ReleaseToken(const CCertificate& rCert, ULONG nTokenId)
	...
	if(pTokenInfo == NULL)
		{
		// token not found
		GetSystem()->GetIOSubSystem()->Output(IOMASK_1|IOMASK_CONST|IOMASK_DEBUG, "IssueCertificate Status 5");
		return false;
		}
...
//--------------------------------------------------------------------------------
static inline int CheckProcessorSerial(const CSSConfigGeneral& config)
	{
#if 0
	...
	TRACE("\
---------------------------------\n\
NOT CHECKING THE SERIAL#\n\
This machine %s\n\
License File %s\n---------------------------------\n", (LPCTSTR) sTemp, (LPCTSTR) config.m_sP3Serial);
#else
...
bool CDBSubSystem::LoadFromFile()
	...
	if(! file.Open(GetSystem()->GetLicenseFilename(), CFile::modeRead|CFile::shareExclusive))
		{
		GetSystem()->GetIOSubSystem()->FormatOutput(IOMASK_1, "Failed to open: %s\n", (LPCTSTR) GetSystem()->GetLicenseFilename());
		return false;
		}
	...
	int nReturn = CheckProcessorSerial(config);

	if(nReturn != 0)
		return LicenseFileFailed(nReturn);
...
bool CDBSubSystem::LicenseFileFailed(int nStatus)
	...
	GetSystem()->GetIOSubSystem()->FormatOutput(IOMASK_1|IOMASK_CONST|IOMASK_DEBUG, "ERR: " + sTemp);
	CSSQuickMail* pMail = new CSSQuickMail;
