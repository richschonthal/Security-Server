...
//--------------------------------------------------------------------------------
CLicenseCheckThread::CLicenseCheckThread(CSecuritySystem* pParent)
		: CTypedThreadObject<CSecuritySystem, CSecuritySystem>(pParent)
...
void CLicenseCheckThread::DoChecking()
	...
	if((ctNow - m_ctLastCheckLicFile).GetDays() > 0)
		...
		LONGLONG nDaysTilExpire = (config.m_ctExpiration - ctNow).GetDays();
		if(nDaysTilExpire <= 5)
			{
			if(nDaysTilExpire < 1)
				{
	...
	if(GetSystem()->GetDongleDeadlineTime() > CTime::GetCurrentTime() || ! GetSystem()->IsServing())
		return;
...
//			return;
		...
		__time64_t* pLastRun = (__time64_t*) &nMem[96];
