...
CSystemMonitorThread::CSystemMonitorThread(CMonitorSubSystem* pParent)
		: CTypedThreadObject<CSecuritySystem, CMonitorSubSystem>(pParent)
...
bool CSystemMonitorThread::Init()
	...
	if(m_pSocket->Create(config.m_nStatusPort) == 0)
		{
		GetSystem()->GetIOSubSystem()->FormatOutput(IOMASK_ERR, "Failed to create monitor socket (%ld)", config.m_nStatusPort);
		return false;
		}
