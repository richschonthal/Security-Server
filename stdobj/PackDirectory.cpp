	...
	while(entry.Read(file))
		...
		switch(entry.m_nType)
			...
			case Entry::TypeFile:
				...
				char temp[1024];
				// could lose something here and cause a nice bug
				// oh well - this code is old and I "just need to get it working"
				// for the APL project - RS 2005
				int nRemaining = (int) entry.m_nLen;
				...
				if(entry.m_nAttr != 0xffffffff)
					...
					::SetFileTime((HANDLE) out, &entry.m_ftCreate, &entry.m_ftAccess, &entry.m_ftWrite);
					}
				out.Close();
				}
				break;
