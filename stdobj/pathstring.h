...
class CPathString : public CString  
	...
	public:
		...
		// return levels by index ie c:\\folder\\hey\\what
		// GetPathnamePart(0) returns folder, GetPathnamePart(1) return hey etc
		CString GetPathnamePart(unsigned int);
		// GetPathnamePartReverse(0) return what, GetPathnamePartReverse(1) return hey etc
		CString GetPathnamePartReverse(unsigned int);
	};
