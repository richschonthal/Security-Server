...
void PrintHeader()
	...
	printf("Copyright (c) 2000 @COMPANY\n");
	printf("usage:\n");
	}

//--------------------------------------------------------------------------------
int tokencompiler(int argc, TCHAR* argv[])
	{
	::SetConsoleTitle("SecurityServer TokenCompiler Version 1.5");
...
bool WriteFile(CFile& fOut, CFile& fOutBkup)
	...
	while(bRv)
		...
		if(g_nExpires == 0)
			{
			printf("error - expiration date must be specified\n");
			bRv = false;
			break;
			}
		...
		if(nLen >= 64)
			{
			printf("error email total len >= 64\n");
			bRv = false;
			break;
			}
		...
		if(nLen >= 64)
			{
			printf("error monitor username total len >= 64\n");
			bRv = false;
			break;
			}
		if(nLen == 0)
			{
			printf("error monitor username must be specified\n");
			bRv = false;
			break;
			}
		memcpy(&temp[nIndex], &nLen, sizeof(nLen));
		...
		if(nLen >= 64)
			{
			printf("error monitor password total len >= 64\n");
			bRv = false;
			break;
			}
		if(nLen == 0)
			{
			printf("error monitor password must be specified\n");
			bRv = false;
			break;
			}
		memcpy(&temp[nIndex], &nLen, sizeof(nLen));
		...
		if(g_nNextToken == 0)
			{
			printf("error at least one token must be specified\n");
			bRv = false;
			break;
			}
...
int _tmain(int argc, TCHAR* argv[], TCHAR* envp[])
{
	...
	{
		nRetCode = tokencompiler(argc, argv);
	}
